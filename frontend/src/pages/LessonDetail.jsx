...
export default function LessonDetail() {
  ...
  const { data: lesson, isLoading: lessonLoading } = useQuery({
    ...
    queryFn: () => apiClient.lessons.getById(lessonId),
    enabled: !!lessonId,
  });

  const { data: progressData } = useQuery({
    queryKey: ["userProgress", user?.email, lessonId],
    queryFn: () => apiClient.userProgress.list({ 
  ...
  const markCompleteMutation = useMutation({
    mutationFn: async () => {
      if (progress) {
        return apiClient.userProgress.update(progress.id, { completed: true, progressPercentage: 100 });
      } else {
        return apiClient.userProgress.upsert({
          userEmail: user.email,
          lessonId,
          completed: true,
  ...
  if (!lessonId) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-green-50/30 to-coral-50/20 flex items-center justify-center">
        <p className="text-gray-600">Invalid lesson selected</p>
      </div>
    );
  }
